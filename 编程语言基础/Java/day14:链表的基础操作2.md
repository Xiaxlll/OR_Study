...
- 解决方法：使用`mv LinkList1.java LinkList1.java.bak`命令将LinkList1.java`和`LinkList2.java`两个`.java`文件备份，使他们不要以`.java`结尾。这样在同一个文件夹中就只有一个`LinkList`类了，这里主类无所谓。这个冲突的原因大概是同一文件夹不同程序中的LinkList class在编译后都是LinkList.class然后就会冲突。
